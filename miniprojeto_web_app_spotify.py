...
def main():
    ...
    st.write('Fonte: API do Spotify (https://developer.spotify.com/documentation/web-api)')
    nome_artista = st.text_input('Qual artista você deseja consultar?')
    if not nome_artista:
        st.stop()
    ...
    if not artista:
        st.warning(f'Artista não encontrado')
        st.stop()
    ...
    for musica in melhores_musicas:
        ...
        link_markdown = f'[{nome_musica}]({link_musica})'
        st.write(f'{link_markdown} (pop: {popularidade_musica})')
